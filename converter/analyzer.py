...
class Analyzer:
    ...
    def analyze(self, archive: ModArchive) -> AnalysisResult:
        ...
        result = AnalysisResult(metadata=metadata, minecraft_version=self.minecraft_version)

        resources = self._collect_resources(archive, namespace)
        result.resources = resources

        lang, lang_by_locale = self._collect_lang(archive, namespace)
        result.lang = lang
        result.lang_by_locale = lang_by_locale
        result.sounds = self._collect_sounds(archive, namespace)

        item_models, item_textures = self._index_item_sources(archive, namespace)
        blockstates, block_models, block_textures = self._index_block_sources(archive, namespace)

        self._build_items(result, item_models, item_textures, lang)
        self._build_modern_item_definitions(result, archive, namespace, lang)
        ...
        self._build_blocks(result, blockstates, block_models, block_textures, lang)
        self._build_recipes(result, archive, namespace)
        self._build_loot(result, archive, namespace)
        self._build_tags(result, archive, namespace)
        self._link_crop_semantics(result, archive, namespace)
        self._ensure_crop_loot(result, namespace)
    ...
    # --- resources --------------------------------------------------------
    def _collect_resources(self, archive: ModArchive, namespace: str) -> list[ResourceNode]:
        nodes: list[ResourceNode] = []
        for name in archive.names:
            kind = self._classify_path(name, namespace)
            if kind is None:
                continue
    ...
    @staticmethod
    def _classify_path(name: str, namespace: str) -> str | None:
        """Classify only resources needed for the CraftEngine resource pack.

        Content-bearing assets (textures/models/blockstates/sounds/particles/
        fonts/lang) are collected; datapack files (recipes/loot/tags) are only
        parsed into IR, not copied. Unknown paths under assets/ or data/ are
        ignored to avoid pulling in every asset of the mod.
        """
        if name.startswith(f"assets/{namespace}/"):
            rest = name[len(f"assets/{namespace}/"):]
            # Keep the complete source asset tree. Minecraft/CraftEngine model
            # references can point to less-common asset domains (for example
            # `items/` item-model definitions, atlases, shaders, equipment,
            # sounds, etc.). Dropping those files causes valid YAML references
            # to point at missing resources.
            if rest:
                return "asset"
            return None
        if name.startswith(f"data/{namespace}/"):
            rest = name[len(f"data/{namespace}/"):]
    ...
    def _collect_lang(
        ...
        for name in ordered_names:
            if name.startswith(f"assets/{namespace}/lang/") and name.endswith(".json"):
    ...
    def _collect_sounds(self, archive: ModArchive, namespace: str) -> dict[str, Any]:
        path = f"assets/{namespace}/sounds.json"
        data = archive.read_json(path)
    ...
    def _link_crop_semantics(self, result: AnalysisResult, archive: ModArchive, namespace: str) -> None:
        ...
        for crop_path, info in relations.items():
            block = result.blocks.get(f"{namespace}:{crop_path}")
            if block is None and not crop_path.endswith("_crop"):
                block = result.blocks.get(f"{namespace}:{crop_path}_crop")
            if block is None:
                continue
    ...
    def _ensure_crop_loot(self, result: AnalysisResult, namespace: str) -> None:
    ...
    # --- recipes ----------------------------------------------------------
    def _build_recipes(self, result: AnalysisResult, archive: ModArchive, namespace: str) -> None:
        for dirname in ("recipes", "recipe"):
            prefix = f"data/{namespace}/{dirname}/"
            for name in archive.names:
                if not name.startswith(prefix) or not name.endswith(".json"):
                    continue
                rel = name[len(prefix):-5]
                ...
                if not isinstance(data, dict):
                    continue
                node = self._recipe_from_json(f"{namespace}:{rel}", data, name)
    ...
    # --- loot -------------------------------------------------------------
    def _build_loot(self, result: AnalysisResult, archive: ModArchive, namespace: str) -> None:
    ...
    # --- tags -------------------------------------------------------------
    def _build_tags(self, result: AnalysisResult, archive: ModArchive, namespace: str) -> None:
    ...
    def _link_recipes_to_items(self, result: AnalysisResult) -> None:
        ...
        # attach recipe references to items
        for recipe in result.recipes.values():
            result_id = (recipe.result or {}).get("id")
            if result_id and result_id in result.items:
                result.items[result_id].recipe_references.append(recipe.id)
    ...
    def _build_crop_display_items(self, result: AnalysisResult) -> None:
        ...
        for block in result.blocks.values():
            ...
            for age in sorted(stages, key=lambda x: int(x)):
                ...
                generated_path = f"{block.namespace}:block/custom/{display_base}_stage{age}"
                stage_tree = {"type": "minecraft:model", "path": generated_path}

                # Resolve source model JSON and flatten cross-like dependency
                # parents into a vanilla minecraft:block/cross generation.
                if archive is not None:
                    try:
                        ns, loc = model_ref.split(":", 1)
                        model_file = f"assets/{ns}/models/{loc}.json"
                        raw_model = archive.read_json(model_file)
                    except Exception:
                        raw_model = None
                    if isinstance(raw_model, dict):
                        parent = str(raw_model.get("parent", ""))
                        textures = raw_model.get("textures")
                        if isinstance(textures, dict) and (
                            "cross" in textures or parent.endswith("/template_crop_cross")
                            or parent.endswith(":block/cross") or parent == "minecraft:block/cross"
                        ):
                            cross_texture = textures.get("cross")
                            if isinstance(cross_texture, str):
                                stage_tree["generation"] = {
                                    "parent": "minecraft:block/cross",
                                    "textures": {"cross": cross_texture},
                                }
                                item.model_tree = stage_tree
                            else:
                                item.model = generated_path
                        else:
                            item.model = model_ref
                if item.model_tree is None and item.model is None:
                    item.model = model_ref
