...
class Settings:
    ...
    # shapeless crafting (workbench) recipe.
    recipe_stations: dict[str, str] = field(default_factory=dict)
    ...
    container_items: dict[str, str] = field(default_factory=dict)
...
def _as_settings(data: Any) -> Settings:
    ...
    if isinstance(data.get("recipe_stations"), dict):
        s.recipe_stations = {str(k): str(v) for k, v in data["recipe_stations"].items()}
    ...
    if isinstance(data.get("container_items"), dict):
        s.container_items = {str(k): str(v) for k, v in data["container_items"].items()}
