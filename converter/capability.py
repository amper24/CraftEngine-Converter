...
from .config import KNOWN_RECIPE_STATIONS, KNOWN_RECIPE_STATION_NOTES, Settings
...
def classify_recipe(recipe: RecipeNode, settings: Settings | None = None) -> Decision:
    ...
    elif station == "unknown" and recipe.recipe_type in KNOWN_RECIPE_STATIONS:
        default_station = KNOWN_RECIPE_STATIONS[recipe.recipe_type]
        note = KNOWN_RECIPE_STATION_NOTES.get(recipe.recipe_type, "")
        result = status.UNSUPPORTED
        reason = (
            f"KNOWN_SOURCE_SERIALIZER: '{recipe.recipe_type}' is a recognized third-party "
            f"recipe type but its station is disabled/mapped to unknown. Default target is "
            f"'{default_station}'; set recipe_stations['{recipe.recipe_type}'] to enable it. {note}".strip()
        )
